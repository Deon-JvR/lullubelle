...
const ORDER_PROCESSING_NOTICE = "Orders are processed within 5–10 business days before collection or dispatch.";
const orderRequiresPhysicalFulfilment = (products) => products.some((product) => !/^(gift-voucher-|online-skin-consultation)/i.test(String(product?.id || "")));
const PRODUCT_SORTS = [
...
const renderOrders = () => {
  ...
  $("[data-list='orders']").innerHTML = visibleOrders.map((item) => {
    ...
    const address = parseNested(item.deliveryAddress || item.address || delivery.address || customer.address, {}) || {};
    const billingAddress = parseNested(item.billingAddress, {}) || {};
    const productsValue = parseNested(item.products, []);
    const products = Array.isArray(productsValue) ? productsValue : [];
    const processingNotice = orderRequiresPhysicalFulfilment(products)
      ? `<aside class="order-processing-notice"><strong>${ORDER_PROCESSING_NOTICE}</strong><span>${String(delivery.option || "").toLowerCase() === "collection" ? "Collection is available only after the customer has been notified that the order is ready." : "Delivery transit time begins only after the order has been dispatched."}</span></aside>`
      : "";
    const formatAddress = (value) => typeof value === "string" ? value : [value.streetAddress || value.street, value.suburb, value.city, value.province, value.postalCode || value.postCode].filter(Boolean).join(", ");
    ...
    const statusOptions = [["Pending", "Awaiting payment confirmation"], ["Paid", "Paid"], ["Failed", "Failed"], ["Refunded", "Refunded"], ["Partially Refunded", "Partially refunded"], ["Cancelled", "Cancelled"]];
...
document.addEventListener("change", async (event) => {
  const input = event.target;
  if (input.matches("[data-order-filter]")) {
    state.orderFilter = input.value;
    renderOrders();
    return;
  }
  if (input.matches("[data-order-select], [data-order-select-all]")) {
    if (input.matches("[data-order-select-all]")) {
      $$('[data-order-select]:not(:disabled)').forEach((box) => { box.checked = input.checked; });
    }
    const count = $$('[data-order-select]:checked').length;
    const button = $("[data-archive-selected]");
    if (button) button.textContent = `Archive selected (${count})`;
    return;
  }
...
document.addEventListener("click", async (event) => {
  ...
  const archiveButton = target.closest?.("[data-order-archive]");
  if (archiveButton) {
    const card = archiveButton.closest("[data-record='orders']");
  ...
  if (target.matches("[data-archive-selected]")) {
